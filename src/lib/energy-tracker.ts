...
// Sample data
export function initializeSampleEnergyData(): void {
  const entries = getEnergyEntries();
  if (entries.length > 0) return;
  
  const activities: ActivityType[] = ["deep_work", "meetings", "admin", "creative", "learning", "exercise", "rest"];
  ...
  for (let i = 0; i < 14; i++) {
    ...
    for (let j = 0; j < numEntries; j++) {
      ...
      // Higher energy in morning, lower in evening
      let baseEnergy = 3;
      if (timeOfDay === "morning") baseEnergy = 4;
      if (timeOfDay === "evening") baseEnergy = 2;
      
      // Exercise and rest boost energy
      if (activity === "exercise" || activity === "rest") baseEnergy += 1;
      // Admin and meetings drain energy
      if (activity === "admin" || activity === "meetings") baseEnergy -= 0.5;
      
      const energyLevel = Math.max(1, Math.min(5, Math.round(baseEnergy + (Math.random() - 0.5)))) as EnergyLevel;
      ...
      addEnergyEntry({
        ...
        energyLevel,
        activity,
        activityName: ACTIVITY_LABELS[activity].label,
        duration: 30 + Math.floor(Math.random() * 90),
        mood: energyLevel >= 4 ? "good" : energyLevel >= 3 ? "neutral" : "bad",
        productivity: energyLevel >= 4 ? "high" : energyLevel >= 3 ? "medium" : "low",
        tags: [],
      });
    }
  }
