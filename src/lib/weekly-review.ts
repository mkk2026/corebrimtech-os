...
// Sample review for first-time users
export const SAMPLE_REVIEW: Omit<WeeklyReview, "id"> = {
  weekStarting: getWeekStart(new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)),
  completedAt: new Date(Date.now() - 6 * 24 * 60 * 60 * 1000).toISOString(),
  wins: [
    "Closed first enterprise client",
    "Shipped v2.0 of the product",
    "Hired senior developer",
  ],
  challenges: [
    "Integration took longer than expected",
    "One prospect ghosted after proposal",
  ],
  lessonsLearned: [
    "Always pad integration timelines by 30%",
    "Follow up within 24 hours of proposals",
  ],
  energyLevel: 4,
  focusLevel: 3,
  goalsProgress: [
    { goalId: "1", goalTitle: "Close 3 enterprise clients", progress: 33 },
    { goalId: "2", goalTitle: "Launch v2.0", progress: 100 },
  ],
  dealsMoved: 2,
  proposalsSent: 3,
  meetingsHeld: 8,
  ...
  goalsForNextWeek: [
    "Close 1 more enterprise client",
    "Ship 2 minor features",
  ],
  blockersToRemove: [
    "Get legal review of new contract",
  ],
  aiInsights: [
    "Strong week with 3 major wins. Focus on maintaining momentum.",
    "Energy was high - identify what contributed to this.",
  ],
};

export function initializeSampleReview(): void {
  const reviews = getWeeklyReviews();
  if (reviews.length === 0) {
    const newReview: WeeklyReview = {
      ...SAMPLE_REVIEW,
